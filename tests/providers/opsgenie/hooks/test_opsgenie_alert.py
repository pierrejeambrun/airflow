...
import pytest
from opsgenie_sdk import AlertApi, CreateAlertPayload, SuccessResponse
...
class TestOpsgenieAlertHook(unittest.TestCase):
    ...
    def test_payload_and_response(self, create_alert_mock):
        hook = OpsgenieAlertHook(opsgenie_conn_id=self.conn_id)
        create_alert_mock.return_value = SuccessResponse(
            **self._mock_success_response_body, api_client=hook.alert_api_instance.api_client
        )
        resp = hook.execute(payload=self._payload)
        create_alert_mock.assert_called_once_with(CreateAlertPayload(**self._payload))
        assert resp.took == self._mock_success_response_body['took']
        assert resp.request_id == self._mock_success_response_body['request_id']
        assert resp.result == self._mock_success_response_body['result']
